...
class EllipticCurve:
    ...
    def print_group_addition_table(self):
        ...
        print(f"ДЕМОНСТРАЦИЯ ГРУППОВОГО ЗАКОНА КРИВАЯ Y^2 = X^3 + {
              self.A}X + {self.B} НАД ПОЛЕМ F_{self.P}")
        print(f"Все найденные точки ({len(all_points)}): {all_points}")

        point_str_len = max(len(str(p)) for p in all_points)
        formatted_points_header = [
            f"{str(p):<{point_str_len}s}" for p in all_points]
        ...
        print(
            "Для каждого элемента есть обратный, сумма которых равна нулевому элементу (O).")
        print("Например:")
        if len(all_points) > 1:
            ...
            if found_neg_P:
                print(
                    f"{P_example} + {found_neg_P} = {self.add_points(P_example, found_neg_P)}")
            else:
                print(f"Обратная точка для {P_example} ({
                      neg_P_example}) не найдена в списке точек (возможна ошибка в реализации).")
        else:
    ...
    def print_generator_table(self):
        ...
        print(f"ПОНЯТИЕ ГЕНЕРАТОРА В ГРУППЕ ТОЧЕК НА ЭЛЛИПТИЧЕСКОЙ КРИВОЙ Y^2 = X^3 + {
              self.A}X + {self.B} НАД ПОЛЕМ F_{self.P}")
        ...
        header_line = "P" + " |" + \
            "".join(f"{h:<{max_kP_len}s}" for h in formatted_kP_headers)
        print(header_line)
        print("-" * len(header_line))
